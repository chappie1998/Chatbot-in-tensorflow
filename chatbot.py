...
with open("data/encoder_data.txt", 'r') as f:
    encoder_inputs = f.readlines()
    encoder_inputs = encoder_inputs[:30000]

with open("data/decoder_data.txt", 'r') as f:
...
embed_layer = embedding_layer_creater(vocab_size, embedding_dim, max_length_inp, embedding_matrix)
...
class Encoder(tf.keras.Model):
    ...
    def call(self, x, hidden, embed_layer):
        x = embed_layer(x)
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, hidden, enc_output, embed_layer):
...
checkpoint_dir = './training_checkpoint_test'
...
for epoch in range(EPOCHS):
    ...
    for (batch, (inp, targ)) in enumerate(dataset):
        ...
        with tf.GradientTape() as tape:
            enc_output, enc_hidden = encoder(inp, hidden, embed_layer)
            ...
            for t in range(1, targ.shape[1]):
                ...
                predictions, dec_hidden, _ = decoder(dec_input, dec_hidden, enc_output, embed_layer)
...
def evaluate(sentence, encoder, decoder, max_length_inp, max_length_tar):
    ...
    enc_out, enc_hidden = encoder(inputs, hidden, embed_layer)

    dec_hidden = enc_hidden
    dec_input = tf.expand_dims([word2idx['<BOS>']], 0)

    for t in range(max_length_tar):
        predictions, dec_hidden, attention_weights = decoder(dec_input, dec_hidden, enc_out, embed_layer)
